...
function Send-Text($ctx, $status, $text) {
  $bytes = [Text.Encoding]::UTF8.GetBytes([string]$text)
  $ctx.Response.StatusCode=$status; $ctx.Response.ContentType='text/plain; charset=utf-8'; $ctx.Response.Headers.Add('Access-Control-Allow-Origin','*'); $ctx.Response.Headers.Add('Access-Control-Allow-Headers','Content-Type, Accept'); if($ctx.Request.Headers['Access-Control-Request-Private-Network'] -eq 'true'){$ctx.Response.Headers.Add('Access-Control-Allow-Private-Network','true')}; $ctx.Response.Headers.Add('Access-Control-Allow-Methods','POST, OPTIONS, GET'); $ctx.Response.Headers.Add('Access-Control-Max-Age','600'); $ctx.Response.OutputStream.Write($bytes,0,$bytes.Length); $ctx.Response.Close()
}
function Get-QzFile($name) {
  $desktop=[Environment]::GetFolderPath('Desktop')
  $path=Join-Path $desktop ('QZ Tray Demo Cert\'+$name)
  if(-not (Test-Path -LiteralPath $path)){throw "QZ file not found: $path"}
...
while($listener.IsListening){
  try{
    ...
    if($ctx.Request.HttpMethod -eq 'GET'){
      if($ctx.Request.Url.AbsolutePath -eq '/qz/certificate'){Send-Text $ctx 200 (Get-QzFile 'digital-certificate.txt');continue}
      if($ctx.Request.Url.AbsolutePath -eq '/qz/private-key'){Send-Text $ctx 200 (Get-QzFile 'private-key.pem');continue}
